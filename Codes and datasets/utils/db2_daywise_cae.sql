...
select 
    ...
    `tweets`.`tdate` AS `tdate`,
    count(*) AS `count`,
    sum(`tweets`.`fposemo`) AS `fposemo`,
    sum(`tweets`.`fnegemo`) AS `fnegemo`,
    sum(`tweets`.`fposemoji`) AS `fposemoji`,
    sum(`tweets`.`fnegemoji`) AS `fnegemoji`,
    sum(`tweets`.`fdeg_adv`) AS `fdeg_adv`,
    sum(`tweets`.`fadv_cnt`) AS `fadv_cnt`,
    sum(`tweets`.`fperiod`) AS `fperiod`,
    sum(`tweets`.`fqmark`) AS `fqmark`,
    sum(`tweets`.`fexclaim`) AS `fexclaim`,
    sum(`tweets`.`f3dots`) AS `f3dots`,
    sum(`tweets`.`ffct1`) AS `ffct1`,
    sum(`tweets`.`ffct2`) AS `ffct2`,
    sum(`tweets`.`ffct3`) AS `ffct3`,
    sum(`tweets`.`ffct4`) AS `ffct4`,
    sum(`tweets`.`ffct5`) AS `ffct5`,
    sum(`tweets`.`ffct6`) AS `ffct6`,
    sum(`tweets`.`ffct7`) AS `ffct7`,
    sum(`tweets`.`ffct8`) AS `ffct8`,
    sum(`tweets`.`ffct9`) AS `ffct9`,
    sum(`tweets`.`ffct10`) AS `ffct10`,
    sum(`tweets`.`ffct11`) AS `ffct11`,
    sum(`tweets`.`ffct12`) AS `ffct12`,
    sum(`tweets`.`ffct13`) AS `ffct13`,
    sum(`tweets`.`ffct14`) AS `ffct14`,
    sum(`tweets`.`ffct15`) AS `ffct15`,
    sum(`tweets`.`fsat_mean`) AS `fsat_mean`,
    sum(`tweets`.`fsat_contrast`) AS `fsat_contrast`,
    sum(`tweets`.`fbrit_mean`) AS `fbrit_mean`,
    sum(`tweets`.`fbrit_contrast`) AS `fbrit_contrast`,
    sum(`tweets`.`fwarm_cool`) AS `fwarm_cool`,
    sum(`tweets`.`fclear_dull`) AS `fclear_dull`,
    sum(`tweets`.`freplies_cnt`) AS `freplies_cnt`,
    sum(`tweets`.`fretweets_cnt`) AS `fretweets_cnt`,
    sum(`tweets`.`flikes_cnt`) AS `flikes_cnt`,
    ...
    sum(rposemo) rposemo,
    sum(raffect) raffect,
    sum(rleisure) rleisure,
    sum(rsad) rsad,
    sum(rdeath) rdeath,
    sum(rauxverb) rauxverb,
    sum(ranger) ranger,
    sum(rnegemo) rnegemo,
    sum(posemoji) rposemoji,
    sum(negemoji) rnegemoji,
    ...
    from (
        ...
        left join(
            select 
            reply_to as rreply_to,
            sum(family) rfamily,
            sum(friend) rfriend,
            sum(posemo) rposemo,
            sum(affect) raffect,
            sum(leisure) rleisure,
            sum(sad) rsad,
            sum(death) rdeath,
            sum(auxverb) rauxverb,
            sum(anger) ranger,
            sum(negemo) rnegemo,
            sum(posemoji) rposemoji,
            sum(negemoji) rnegemoji
        ...
        on tweets.id_str = reps.rreply_to
    ) as tweets
    group by `tweets`.`user_id`,`tweets`.`tdate` 
...
(
    ...
    sum(`tweets`.`pos`) AS 'ppos',
    sum(`tweets`.`neg`) AS 'pneg',
    sum(`tweets`.`neu`) AS 'pneu'
    from `tweets`
    group by `tweets`.`user_id`,`tweets`.`tdate` 
    having `tweets`.`user_id` 
    in (select `users`.`user_id` from `users`)
...
and (cur.tdate = prev.ptdate + interval 1 day) 
